...
import { ILineData } from "src/models/ITimeSeriesData";
...
const xScale = (data: ILineData[], w = WIDTH, m = MARGIN) => {
  const xScale = d3
    .scaleLinear()
    .domain(d3.extent(data, (d: ILineData) => d.x))
    .nice()
    .range([m, w - m]);

  console.log(
    "xExtent:",
    d3.extent(data, (d: ILineData) => d.x),
  );
  return xScale;
};

const yScale = (data: ILineData[], h = HEIGHT, m = MARGIN) => {
  const yScale = d3
    .scaleLinear()
    .domain(d3.extent(data, (d: ILineData) => d.y))
    .range([h - m, m]);

  console.log(
    "yExtent:",
    d3.extent(data, (d: ILineData) => d.y),
  );
  return yScale;
};

export class LineAndDotPlot {
  _data1: ILineData[];
  _data2;
  _svg: SVGSVGElement;
  _title = "";
  ...
  plot() {
    ...
    console.log("TimeSeries1: plot: points1:", points1, this._color);
   ...
   */
  _drawAxisAndLabels() {
    console.log(`TimeSeries5:_drawAxisAndLabels:`);
