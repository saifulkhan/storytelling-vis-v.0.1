...
export function getNations(): string[] {
  return ["England", "Wales", "Northern Ireland", "Scotland"].sort();
}
...
/*********************************************************************************************************
 * Filter/select nation data
 *********************************************************************************************************/
let nation;
let annotations;
let casesData;

export async function filterData(_nation) {
  nation = _nation;

  const calendarEvents = await prepareCalendarEvents(nation);
  annotations = prepareAnnotations(nation, calendarEvents);
  // prettier-ignore
  console.log("filterData: maxCounter = ", maxCounter, ", annotations = ", annotations);

  casesData = nationCases[nation];
}
...
export async function createScrollingSvg(selector) {
  ...
  function onScrollCallback(_counter: number) {
    counter = _counter;
    console.log("onScrollCallback: counter = ", counter);

    timeline.annotations(annotations).plot(counter);
    timeseries.animate(counter);
  }

  timeline.annotations(annotations).plot(0);
  timeseries.animate(0);
}

export function animateTimeSeries(inc) {
  // prettier-ignore
  console.log("animateTimeSeries: counter = ", counter, ", inc = ", inc);
  ...
  console.log("animateTimeSeries: counter = ", counter);
